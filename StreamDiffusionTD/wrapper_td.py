...
class StreamDiffusionWrapper:
    ...
    def _load_model(
        ...
        #                 print(f"\n\033[36mDownloading {lora_name}\033[0m")
        #                 lora_name = download_model_file(lora_name, cache_dir=os.getcwd() + "/models/LoRA")
        #             adapter_name = os.path.splitext(os.path.basename(lora_name))[0]
        #             stream.load_lora(lora_name, adapter_name)
        #             stream.fuse_lora(lora_scale=lora_scale)
        #             self.loaded_loras[adapter_name] = {"path": lora_name, "scale": lora_scale}
        #             print(f"Successfully loaded LoRA model:")
        #             print(f"\033[92m{lora_name}\033[0m\n")
        #     except Exception as e:
        #         error_message = str(e)[:400]
        #         print(f"\n\033[93mERROR loading LoRA Models: {error_message}\033[0m")
        #         if len(str(e)) > 400:
        #             lines_shortened = len(str(e)[400:].split('\n'))
        #             print(f"\033[93m... (error message shortened by {lines_shortened} lines)\033[0m")
    ...
    def update_lora_weights(self, lora_dict: Dict[str, float], print_info: bool = False):
        try:
            active_adapters = self.stream.pipe.get_active_adapters()

            # Unload LoRAs that are no longer in the lora_dict
            for adapter_name in list(self.loaded_loras.keys()):
                if adapter_name not in [os.path.splitext(os.path.basename(name))[0] for name in lora_dict.keys()]:
            ...
            for lora_name, lora_scale in lora_dict.items():
                ...
                if adapter_name in self.loaded_loras:
                    if self.loaded_loras[adapter_name]["scale"] != lora_scale:
                        self.loaded_loras[adapter_name]["scale"] = lora_scale
                else:
                    if print_info:
                        print(f"\n\033[36m...Loading additional LoRA Model: {os.path.basename(lora_name)}, Weight: {lora_scale}\033[0m")
                    try:
                        self.stream.pipe.load_lora_weights(lora_name, adapter_name=adapter_name)
                        self.loaded_loras[adapter_name] = {"path": lora_name, "scale": lora_scale}
                        if print_info:
                            print(f"Successfully loaded LoRA model:")
                            print(f"\033[92m{lora_name}\033[0m\n")
                    except Exception as e:
                        print(f"\033[91mError loading LoRA '{adapter_name}': {e}\033[0m")
                        continue
