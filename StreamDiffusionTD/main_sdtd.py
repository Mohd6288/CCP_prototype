    ...
    while True:
        try:
            ...
            if new_lora_weights != lora_dict:
                ...
                update_lora_weights(stream.stream, lora_dict, use_lcm_lora, lcm_lora_id, stream)
            ...
            # Continue with your shared memory operations here
            # if TIME_DEBUG:
            #     timing_stats['image_generation'].append(time.time() - image_generation_start)
            #     postprocessing_start = time.time()

            # # processed_tensor = optimized_postprocess_image(processed_tensor)
            # # processed_np = processed_tensor.cpu().numpy()
            # if mode == "img2img":
            #     processed_np = postprocess_image(processed_tensor)
